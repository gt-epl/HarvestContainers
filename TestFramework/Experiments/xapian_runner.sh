...
if [ $TYPE == "harvest-irq" ] || [ $TYPE == "baseline-irq" ]; then
  SUMMARY_FILE="xapian-irq.summary"
fi
...
if [ ! -f "$CFGFILE" ]; then
  ...
  echo "uuid mean p50 p90 p95 p99 min max" > $RESDIR/$SUMMARY_FILE
...
calcLats() {
  ...
  echo "${LOGFILE} $(python parselats_old.py $RESDIR/$LOGFILE.bin)" | tee -a $RESDIR/$SUMMARY_FILE
