...
runBalancer() {
	...
	echo "sudo taskset -a -c ${BALANCER_BINDCPU} ${BIN_PATH}/${BALANCER_BINARY} ${SECONDARY_PID} ${TARGET_IDLE_CORES} ${MIN_SECONDARY_CORES} ${CPULIST} ${SECONDARY_CPULIST} 2>&1 > /tmp/balancer-debug.log &"
        sudo taskset -a -c ${BALANCER_BINDCPU} ${BIN_PATH}/${BALANCER_BINARY} ${SECONDARY_PID} ${TARGET_IDLE_CORES} ${MIN_SECONDARY_CORES} ${CPULIST} ${SECONDARY_CPULIST} &
    fi
}

runListener() {
    sudo taskset -a -c ${LISTENER_BINDCPU} ${BIN_PATH}/${LISTENER_BINARY} ${CPULIST} > /tmp/listener-debug.log &
}
...
sendPrimaryInfo() {
    python3 ${BIN_PATH}/send_primary_info.py $1 $2 &
}
