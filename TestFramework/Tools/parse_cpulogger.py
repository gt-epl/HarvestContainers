...
def print_time_spent_with_idle():
    times = np.array(list(time_spent_with_idle_cores.values()))[:-1]
    totalTime = np.sum(times)
    percentages = (100*times/totalTime)
    np.set_printoptions(suppress=True, formatter={'float_kind':'{:0.2f}'.format})
    print(np.asarray((np.arange(len(times)), times, percentages)).T)
    #for core in CPULIST:
...
parse_log()
print_active_idle()
percentages = print_time_spent_with_idle()
...
print(np.asarray((unique, counts, pcts)).T)

print("time,count")
print(np.asarray((percentages,pcts)).T)
